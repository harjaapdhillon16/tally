...
export function useDashboardAnalytics() {
  ...
  const trackRangeToggle = useCallback((range: '30d' | '90d') => {
    if (posthog) {
  ...
  const trackAlertClicked = useCallback((type: 'low_balance' | 'unusual_spend' | 'needs_review') => {
    if (posthog) {
  ...
  const trackChartHover = useCallback((chart: 'inout' | 'top5' | 'trend') => {
    if (posthog) {
