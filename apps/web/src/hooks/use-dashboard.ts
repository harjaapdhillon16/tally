...
export function useDashboardCharts(dashboard: DashboardDTO | undefined, timeRange: '30d' | '90d') {
  const chartData = dashboard ? [
    {
      name: 'Cash In',
      [`${timeRange}`]: Number(BigInt(dashboard.inflowOutflow[timeRange === '30d' ? 'd30' : 'd90'].inflowCents)) / 100,
    },
    {
      name: 'Cash Out',
      [`${timeRange}`]: Number(BigInt(dashboard.inflowOutflow[timeRange === '30d' ? 'd30' : 'd90'].outflowCents)) / 100,
    }
  ] : [];

  const colors = ['#0088FE', '#00C49F', '#FFBB28', '#FF8042', '#8884D8'];
  const pieData = dashboard?.topExpenses30.map((expense, index) => ({
    name: expense.name,
