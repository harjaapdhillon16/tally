...
export function useOptimisticUpdates() {
  ...
  const [optimisticState, setOptimisticState] = useState<Map<string, Partial<ReviewTransactionItem>>>(new Map());

  const applyOptimisticUpdate = useCallback((
    transactionId: string, 
    updates: Partial<ReviewTransactionItem>
  ) => {
    setOptimisticState(prev => new Map(prev).set(transactionId, {
  ...
  const getOptimisticTransaction = useCallback((transaction: ReviewTransactionItem): ReviewTransactionItem => {
