...
describe('calculateMetrics', () => {
  ...
  test('calculates latency statistics', () => {
    const transactions = [createTransaction('tx-1'), createTransaction('tx-2'), createTransaction('tx-3')];
    const results = [
...
describe('calculateCalibration', () => {
  test('calculates calibration bins correctly', () => {
    const transactions = [
      { id: 'tx-1', description: 'Test', amountCents: '-100', currency: 'USD', categoryId: 'cat-1' },
      { id: 'tx-2', description: 'Test', amountCents: '-100', currency: 'USD', categoryId: 'cat-2' },
      { id: 'tx-3', description: 'Test', amountCents: '-100', currency: 'USD', categoryId: 'cat-1' },
      { id: 'tx-4', description: 'Test', amountCents: '-100', currency: 'USD', categoryId: 'cat-1' },
    ];

    const results = [
      { id: 'tx-1', predictedCategoryId: 'cat-1', confidence: 0.9, rationale: [], engine: 'pass1' as const, timings: { totalMs: 100 } },
      { id: 'tx-2', predictedCategoryId: 'cat-1', confidence: 0.9, rationale: [], engine: 'pass1' as const, timings: { totalMs: 100 } }, // Wrong prediction
      { id: 'tx-3', predictedCategoryId: 'cat-1', confidence: 0.5, rationale: [], engine: 'pass1' as const, timings: { totalMs: 100 } },
      { id: 'tx-4', predictedCategoryId: 'cat-2', confidence: 0.5, rationale: [], engine: 'pass1' as const, timings: { totalMs: 100 } }, // Wrong prediction
    ];
  ...
  test('returns empty array when no ground truth available', () => {
    const transactions = [{ id: 'tx-1', description: 'Test', amountCents: '-100', currency: 'USD' }];
    const results = [{ id: 'tx-1', predictedCategoryId: 'cat-1', confidence: 0.9, rationale: [], engine: 'pass1' as const, timings: { totalMs: 100 } }];
  ...
  test('exports complete metrics to CSV format', () => {
    const metrics = {
      ...
      accuracy: {
        overall: 0.85,
        perCategory: [
          { categoryId: 'cat-1', accuracy: 0.9, precision: 0.85, recall: 0.95, f1: 0.9, support: 50 },
          { categoryId: 'cat-2', accuracy: 0.8, precision: 0.75, recall: 0.85, f1: 0.8, support: 30 },
        ],
        confusionMatrix: [[45, 5], [6, 24]],
