...
function calculateAccuracy(
  ...
  const resultsWithGroundTruth = results.filter(r => 
    !r.error && 
    r.predictedCategoryId && 
    groundTruthMap.has(r.id)
  );
  ...
  const correct = resultsWithGroundTruth.filter(r => 
    r.predictedCategoryId === groundTruthMap.get(r.id)
  ).length;
  ...
  const perCategory = categoryLabels.map(categoryId => {
    const truePositives = resultsWithGroundTruth.filter(r => 
      r.predictedCategoryId === categoryId && groundTruthMap.get(r.id) === categoryId
    ).length;
    
    const falsePositives = resultsWithGroundTruth.filter(r => 
      r.predictedCategoryId === categoryId && groundTruthMap.get(r.id) !== categoryId
    ).length;
    
    const falseNegatives = resultsWithGroundTruth.filter(r => 
      r.predictedCategoryId !== categoryId && groundTruthMap.get(r.id) === categoryId
    ).length;
    
    const support = resultsWithGroundTruth.filter(r => 
      groundTruthMap.get(r.id) === categoryId
    ).length;
...
export function calculateCalibration(
  ...
  return bins.map(bin => {
    ...
    const avgConfidence = bin.results.reduce((sum, r) => sum + r.confidence!, 0) / bin.results.length;
    const correct = bin.results.filter(r => 
      r.predictedCategoryId === groundTruthMap.get(r.id)
    ).length;
