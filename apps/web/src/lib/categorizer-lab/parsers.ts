 ...
 */
function normalizeAmount(input: string, context: 'csv' | 'json' | 'dollars' | boolean = false): string {
  ...
  for (let i = 0; i < transactions.length; i++) {
    try {
      ...
      if (error instanceof z.ZodError) {
        errors.push(`Transaction ${i + 1}: ${error.issues.map((e: z.ZodIssue) => e.message).join(', ')}`);
      } else {
