...
describe('generateSyntheticData', () => {
  ...
  test('respects mccMix parameter', () => {
    ...
    const restaurantCount = transactions.filter(tx => 
      tx.mcc && restaurantMCCs.includes(tx.mcc)
    ).length;
  ...
  test('respects positive/negative ratio', () => {
    ...
    const expenseCount = nonRevenueTransactions.filter(tx => 
      parseInt(tx.amountCents) < 0
    ).length;
