...
function generateDescription(vendor: VendorTemplate, noisePercent: number, rng: () => number): string {
...
function generateMerchantName(vendor: VendorTemplate, noisePercent: number, rng: () => number): string {
...
export function generateScenario(scenario: 'ambiguous' | 'clear' | 'mixed'): LabTransaction[] {
  switch (scenario) {
    ...
    default:
      return [
        ...generateScenario('clear'),
        ...generateScenario('ambiguous'),
      ];
  }
}