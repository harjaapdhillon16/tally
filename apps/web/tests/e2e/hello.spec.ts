...
test.describe("Nexus Application", () => {
  test("should render homepage with title containing Nexus", async ({
    page,
  }) => {
  ...
  test("should render app shell with sidebar and topbar when visiting dashboard", async ({
    page,
  }) => {
    ...
    if (currentURL.includes("/sign-in") || currentURL.includes("/sign-up")) {
      ...
      const signInForm = page.locator(
        'form, [data-testid="sign-in"], .sign-in',
      );
